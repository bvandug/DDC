...
def calculate_metrics(time_lst, angle_lst, stability_threshold=0.1):
    ...
    if not angle_lst or len(angle_lst) < 2:
        return metrics

    # Overshoot Calculation
    start_angle = angle_lst[0]
    start_sign = np.sign(start_angle)
    ...
    if first_crossing_index != -1:
        ...
        if overshoot_region:
            if start_sign > 0:
                metrics['overshoot_rad'] = abs(min(overshoot_region)) if any(x < 0 for x in overshoot_region) else 0.0
            else:
                metrics['overshoot_rad'] = max(overshoot_region) if any(x > 0 for x in overshoot_region) else 0.0

    # Stabilisation Time & Steady-State Error Calculation
    simulation_time = 1.5
    time_delta = simulation_time / len(time_lst)
    ...
    for i in range(len(angle_lst)):
        if abs(angle_lst[i]) <= stability_threshold:
            ...
            if consecutive_stable_steps >= required_stable_steps:
                ...
                if settled_angles:
                    metrics['steady_state_error_rad'] = np.mean(np.abs(settled_angles))
                break
        else:
            consecutive_stable_steps = 0
            first_stable_time = None
...
def run_simulation():
    ...
    model_name = 'pendulumSimPID'
    mask_name = 'Pendulum'
    simulation_time = 1.5
    noise_levels_to_test = [0.0, 0.001, 0.01, 0.1]
    initial_angles_to_test = [0.4553, -0.9443, 0.1559, -0.6195, -0.8419]
    num_episodes_stochastic = 1
    ...
    for noise_level in noise_levels_to_test:
        ...
        for initial_angle in initial_angles_to_test:
            ...
            # Tertiary Loop: Run multiple episodes for stochastic noise
            for episode in range(1, num_episodes + 1):
                ...
                # Calculate and store results for this episode
                episode_metrics.append(calculate_metrics(time_lst, angle_lst))
                episode_plots.append((time_lst, angle_lst))
            ...
            avg_metrics = {}
            avg_metrics['stab_time_mean'] = np.mean(stab_times) if stab_times else 'N/A'
            avg_metrics['stab_time_std'] = np.std(stab_times) if stab_times else 0
            avg_metrics['overshoot_mean'] = np.mean(overshoots)
            avg_metrics['overshoot_std'] = np.std(overshoots)
            avg_metrics['sse_mean'] = np.mean(sses) if sses else 'N/A'
            avg_metrics['sse_std'] = np.std(sses) if sses else 0
            ...
            # Individual Plot Generation (per angle)
        ...
        ax.legend(loc='best', frameon=True, fancybox=True, shadow=True, facecolor='white', edgecolor='lightgrey')
        ax.grid(False); ax.set_xlim(0, 1.5)
        summary_plot_filename = f'pendulum_summary_noise_{noise_level}.svg'
        plt.tight_layout();
        plt.savefig(summary_plot_filename, bbox_inches='tight');
        plt.close(fig)
