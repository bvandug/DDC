...
def duty_sequence(
    ...
    """Generate a duty command sequence for a given pattern.

    Supports:
      • "constant": duty fixed at `constant_duty`
      • "sine":     `center + amp*sin(2π*freq_hz*t)` clipped to [0.1, 0.9]
      • "step":     `duty_a` until `step_at`, then `duty_b` (both clipped)

    Parameters
    ----------
    mode : {"constant","sine","step"}
        Duty pattern to generate.
    steps : int
        Number of PWM periods (RL steps).
    dt : float
        Base integration step (seconds).
    frame_skip : int
        Number of base steps per PWM period (`T_sw = dt*frame_skip`).
    center, amp, freq_hz : float
        Sine pattern parameters.
    constant_duty : float
        Constant duty for "constant" mode.
    duty_a, duty_b : float
        Pre/post step values for "step" mode.
    step_at : float
        Time (seconds) when the step occurs.

    Returns
    -------
    tuple[np.ndarray, np.ndarray]
        `(t, duty)` arrays of length `steps`.
    """
    Tsw = dt * frame_skip
...
def run_one(env_np, env_sim, duties, sim_extra_steps=1, sim_drop_last=1):
    """Run both environments with the same duty sequence and align traces.

    The NumPy env runs exactly `len(duties)` steps. The Simulink env runs
    `len(duties) + sim_extra_steps`, optionally dropping `sim_drop_last`
    tail points to align timing. If the Simulink env provides a measured
    duty in `info` (keys like `measured_duty`, `gate_frac`), it is stored.

    Parameters
    ----------
    env_np, env_sim :
        Instantiated NumPy and Simulink environments.
    duties : array_like
        Duty command per step in [0.1, 0.9].
    sim_extra_steps : int, optional
        Extra steps to run in Simulink before alignment. Default 1.
    sim_drop_last : int, optional
        Tail points to drop from Simulink trace for alignment. Default 1.

    Returns
    -------
    dict
        Dictionary with aligned arrays: `t`, `duty`, `vC_np`, `vC_sim`,
        `iL_np`, `iL_sim`, `err_np`, `err_sim`, `r_np`, `r_sim`,
        termination/truncation flags, and optional `duty_meas_sim`.
    """
    import numpy as np
    steps = len(duties)
    ...
    for k, d in enumerate(duties):
        ...
        rec_np["r"][k] = float(r)
        rec_np["term"][k] = bool(te)
        rec_np["trunc"][k] = bool(tr)
        if te or tr:
            for key in rec_np:
                rec_np[key] = rec_np[key][: k + 1]
            steps = k + 1
            break
    ...
    rec_sm = {
        "t": np.arange(sim_steps) * Tsw,
        "duty": sim_duties.copy(),
        "vC": np.zeros(sim_steps),
        "iL": np.full(sim_steps, np.nan),
        "err": np.zeros(sim_steps),
        "r": np.zeros(sim_steps),
        "term": np.zeros(sim_steps, bool),
        "trunc": np.zeros(sim_steps, bool),
        "gate_meas": [],
        "duty_meas_sim": np.full(sim_steps, np.nan),
    }
    ...
    for k, d in enumerate(sim_duties):
        ...
        iL = info.get("iL", None)
        rec_sm["iL"][k] = (np.nan if iL is None else float(iL))
        rec_sm["err"][k] = float(info.get("err", o[1]))
        rec_sm["r"][k] = float(r)
        rec_sm["term"][k] = bool(te)
        rec_sm["trunc"][k] = bool(tr)
        if te:
            for key in rec_sm:
                rec_sm[key] = rec_sm[key][: k + 1]
            sim_steps = k + 1
            break
    ...
    return {
        ...
        "iL_sim": rec_sm["iL"][:n],
        "err_np": rec_np["err"][:n],
        "err_sim": rec_sm["err"][:n],
        "r_np": rec_np["r"][:n],
        "r_sim": rec_sm["r"][:n],
        "term_np": rec_np["term"][:n],
        "term_sim": rec_sm["term"][:n],
        "trunc_np": rec_np["trunc"][:n],
        "trunc_sim": rec_sm["trunc"][:n],
        "duty_meas_sim": rec_sm["duty_meas_sim"][:n],
    }


def _moving_average(x: np.ndarray, n: int) -> np.ndarray:
    """Compute a simple moving average of window size `n`.
    ...
    x : np.ndarray
        Input 1-D array.
    n : int
        Window length; if <=1 or longer than x, returns a copy of x.

    Returns
    -------
    np.ndarray
        Smoothed array (length `len(x)-n+1`) or copy of x if disabled.
    """
...
def summarize(rec, target_voltage, ma_window=0):
    """Compute comparison metrics between NumPy and Simulink traces.

    Produces RMSE/MAE of vC, steady-state means (tail window), errors to
    target, delta between SS means, and optional moving-average metrics.
    If measured duty from Simulink is present, includes mean and tail.
    ...
    target_voltage : float
        Reference target voltage (V).
    ma_window : int, optional
        Moving-average window in periods; 0 disables. Default 0.

    Returns
    -------
    dict
        Metrics summary; empty dict if the trace is too short.
    """
    v_np = rec["vC_np"]
    v_sm = rec["vC_sim"]
    n = len(v_np)
        ...
        stats.update(
            {
                ...
                "v_np_ss_ma": float(
                    np.mean(v_np_ma[-max(5, len(v_np_ma) // 10) :])
                ),
                "v_sm_ss_ma": float(
                    np.mean(v_sm_ma[-max(5, len(v_sm_ma) // 10) :])
                ),
            }
        )
    ...
    if dm is not None:
        ...
        if np.any(mask):
            ...
            stats["measured_duty_sim_tail"] = float(
                np.mean(m[mask][-tail:] if tail < mask.sum() else m[mask])
            )
...
def _annotate_textbox(ax, lines, loc="upper right"):
    """Draw a small text box inside `ax` with the provided lines.
    ...
    ax : matplotlib.axes.Axes
        Target axes.
    lines : list[str]
        Lines of text (joined with newlines).
    loc : {"upper right","upper left","lower right","lower left"}, optional
        Anchor location inside the axes. Default "upper right".
    """
    text = "\n".join(lines)
    anchors = {
...
def save_csv(rec, out_csv: Path):
    """Save aligned telemetry to CSV and return the DataFrame.

    Columns include time, duty, both vC/iL, errors, rewards, and flags.
...
def make_plots(rec, target_voltage, title_prefix, out_dir: Path, stats: dict):
    """Create and save comparison plots (duty, vC overlay, ΔvC, iL overlay).

    Uses `stats` for inline annotations (RMSE/MAE, SS window and band).
    Files are saved into `out_dir` with fixed filenames:
      01_duty.png, 02_vc_overlay.png, 03_vc_diff.png, 04_iL_overlay.png
    ...
    rec : dict
        Output of `run_one`.
    target_voltage : float
        Target voltage (V) for reference lines and band.
    title_prefix : str
        Label prefix for figure titles.
    out_dir : pathlib.Path
        Output directory (created if necessary).
    stats : dict
        Summary from `summarize`, used for annotations.
    """
    import matplotlib.pyplot as plt
...
def main():
    """CLI entry point: generate duty, run both envs, save CSV and plots."""
    ...
    ap.add_argument("--sim-extra-steps", type=int, default=0, help="Run Simulink for this many extra steps (uses last duty).")
    ap.add_argument("--sim-drop-last", type=int, default=0, help="After Simulink run, drop this many tail steps before alignment.")
    ap.add_argument("--ma-window", type=int, default=25, help="Moving-average window (in periods) for DC-bias metrics (0 disables).")
    ...
    make_plots(
        rec,
        target_voltage=args.target,
        title_prefix=f"{args.mode.UPPER()} duty" if hasattr(args.mode, "UPPER") else f"{str(args.mode).upper()} duty",
