...
def evaluate_full_metrics(
    ...
    for ep in range(n_episodes):
        res = env.reset()
        obs = res[0] if isinstance(res, tuple) else res  # Gymnasium: (obs, info)
...
def discover_models(algo: str, roots: list[str]) -> dict[str, str]:
    """Find all *.zip models containing the algo name in their path."""
    algo_l = algo.lower()
    files = []
    for root in roots:
        files.extend(glob.glob(os.path.join(root, "**", "*.zip"), recursive=True))

    picked = {}
    for p in files:
        pl = p.lower()
        if algo_l in pl:
            # derive a friendly run name (prefer parent dir if it encodes noise)
            parent = os.path.basename(os.path.dirname(p))
            base = os.path.splitext(os.path.basename(p))[0]
            label = parent if re.search(r"(noise|sigma|std)", parent, re.I) else base
            picked[label] = p
    return picked
...
if __name__ == "__main__":
    ...
    parser.add_argument("--algo", required=True,
                        help="Algorithm name: dqn, ddpg, ppo, a2c, sac, td3")
    parser.add_argument("--roots", nargs="*", default=["."],
                        help="Folders to search recursively for *.zip models")
    ...
    if algo_key not in {"DQN", "DDPG", "PPO", "A2C", "SAC", "TD3"}:
        raise SystemExit(f"Unknown algo: {args.algo}")

    models = discover_models(algo_key, args.roots)
    if not models:
        raise SystemExit(f"No models found for '{algo_key}' under {args.roots}")

    ModelClass = globals()[algo_key]
    results = {}

    for run_name, path in sorted(models.items()):
        ...
        # Load model (cpu for on-policy to avoid CUDA surprises)
        model = (ModelClass.load(path, device="cpu")
                 if algo_key in ("PPO", "A2C")
                 else ModelClass.load(path))

        results[run_name] = evaluate_full_metrics(
            model,
            env,
            ...
            output_dir=os.path.join(
                "plots",
                algo_key,
                f"{args.algo.lower()}_env_noise_{args.env_noise:.3f}",
                run_name,
            ),
