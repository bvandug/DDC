...
def main():
    parser = argparse.ArgumentParser(
        description="Read sim‐vs‐NumPy‐vs‐JAX CSV and save PDFs in degrees and radians."
    )
    parser.add_argument(
        "csvfile",
    ...
    fig.tight_layout()
    fig.savefig("theta_deg.pdf", format="pdf")
    plt.close(fig)
    ...
    ax.legend(loc="upper left"); ax.grid(True)
    fig.tight_layout()
    fig.savefig("theta_rad.pdf", format="pdf")
    plt.close(fig)
    ...
    ax.legend(loc="upper left"); ax.grid(True)
    fig.tight_layout()
    fig.savefig("theta_dot_deg.pdf", format="pdf")
    plt.close(fig)
    ...
    ax.legend(loc="upper left"); ax.grid(True)
    fig.tight_layout()
    fig.savefig("theta_dot_rad.pdf", format="pdf")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig("abs_diff.pdf", format="pdf")
    plt.close(fig)
