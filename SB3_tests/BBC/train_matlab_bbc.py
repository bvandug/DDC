...
class FancyTensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        self.pbar.update(1)
        # checkpointing
        if (self.num_timesteps in self.save_steps) and (
            self.num_timesteps not in self.saved_steps
        ):
...
def main():
    ...
    model.learn(
        total_timesteps=args.timesteps, tb_log_name=f"run_noise_{args.noise_std:0.3f}"
    )
