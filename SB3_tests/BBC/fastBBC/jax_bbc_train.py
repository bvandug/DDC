...
from bb_c_env import BuckBoostCEnv
...
def main():
    ...
    if args.algo == "a2c":
        ...
        policy_kwargs = dict(net_arch=[400, 400], activation_fn=nn.Tanh)  # smaller net = faster
        model = SAC(
            "MlpPolicy",
            env,
            learning_rate=0.00010502105436744271,
            buffer_size=200_000,
            batch_size=64,
            tau=0.00834110643236209,
            gamma=0.9857514384317185,            # adjust for macro-k
            train_freq=2,
            gradient_steps=16,
            learning_starts= 10000, #if args.macro_k == 1 else 15_000,
            ent_coef="auto",        # sensible initial alpha
