...
def define_hyperparameters(trial: optuna.Trial, algo_name: str):
    ...
    if algo in ["a2c", "ppo"]:
        # General network parameters for on-policy algorithms
        ...
        # Using a more comprehensive search space for SAC
        n_layers = trial.suggest_int("n_layers", 1, 3)
    ...
    # --- START: MODIFIED TD3 BLOCK ---
        ...
        params = {
            ...
            "policy_kwargs": policy_kwargs,
            # Retaining these from the original script for a more robust search
        ...
        return params
    # --- END: MODIFIED TD3 BLOCK ---

    elif algo == "ddpg":
        # This block is now self-contained
        n_layers = trial.suggest_int("n_layers", 1, 3)
        layer_size = trial.suggest_int("layer_size", 64, 512, log=True)
        activation_fn_name = trial.suggest_categorical("activation_fn", ["tanh", "relu"])
        activation_fn = {"tanh": nn.Tanh, "relu": nn.ReLU}[activation_fn_name]
...
def tune_hyperparameters(algo_name, n_trials=50, n_jobs=1):
    ...
    """
    # UPDATED: Changed the study name to avoid conflicts with old databases.
...
if __name__ == "__main__":
    # To run the new TD3 tuning, change the algorithm here
