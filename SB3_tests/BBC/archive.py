...
class JAXBuckBoostConverterEnv(gym.Env):
    ...
    def _integrate_substep(self, u: int, iL: float, vC: float) -> tuple[float, float]:
        """One Euler substep of the converter dynamics.
        u = 1 (switch ON), u = 0 (switch OFF/diode ON).
        Includes optional DCM handling to avoid negative inductor current.
        """
        dt = self.dt
        L  = self.L
        C  = self.C
        ...
        if u == 1:
            # ON: inductor connected to Vin, diode reverse-biased, load fed by capacitor
            diL = (self.Vin - self.Ron_sw * iL) / L
            dvC = ( - vC / R ) / C
            iL_new = iL + dt * diL
            vC_new = vC + dt * dvC
            # ON phase should not produce negative iL; guard anyway
            if self.enforce_dcm and iL_new < 0.0:
                iL_new = 0.0
            return iL_new, vC_new

        # OFF: inductor dumps into (cap + load) through diode (if iL>0)
        diL_off = (vC - self.Vf - self.Ron_d * iL) / L

        if self.enforce_dcm and diL_off < 0.0:
            # Would iL cross zero within this substep?
            if iL + dt * diL_off <= 0.0 and iL > 0.0:
                # Time to reach zero current
                dt1 = iL / (-diL_off)  # fraction of dt until iL hits 0
                dt1 = float(np.clip(dt1, 0.0, dt))

                # Part 1: diode ON until iL -> 0
                # Approximate dvC using starting iL (small dt1)
                dvC_on = (-iL - vC / R) / C
                vC_mid = vC + dt1 * dvC_on

                # Now iL is zero; Part 2: diode OFF (no inductor conduction)
                dt2 = dt - dt1
                if dt2 > 0.0:
                    dvC_off = (-vC_mid / R) / C
                ...
                return 0.0, vC_new
            # Else: iL stays positive across the substep; regular OFF update

        dvC = (-iL - vC / R) / C
        iL_new = iL + dt * diL_off
        vC_new = vC + dt * dvC

        # Final clamp to prevent numerical drift slightly below zero
        if self.enforce_dcm and iL_new < 0.0:
            iL_new = 0.0
            # If we snapped to zero, recompute dvC with zero inductor for this tiny overshoot
            vC_new = vC + dt * ((-vC / R) / C)
    ...
    def step(self, action):
        ...
        self.prev_state = self.state.copy()
        prev_vC = float(self.prev_state[1])
        prev_error = self.prev_error

        # Resolve PWM over one full switching period: first D*frame_skip substeps ON, then OFF
        iL, vC = float(self.state[0]), float(self.state[1])
        on_steps = int(np.round(duty * self.frame_skip))
        for k in range(self.frame_skip):
            u = 1 if k < on_steps else 0
            iL, vC = self._integrate_substep(u, iL, vC)
            self.time += self.dt
        ...
        # Reward (shaping only; no termination here)
        reward = self.calculate_reward(duty)

        # --- termination / truncation (single source of truth) ---
        terminated = False
        truncated  = False
        ...
        "dduty": float(duty - self.prev_duty),
        "in_band": bool(abs(abs(vC) - abs(self.target_voltage)) <= self._band_e * abs(self.target_voltage)),
        # NEW: duty quantization telemetry
        "duty_cmd": float(duty),
